...
from exchange.venues import DEFAULT_VENUES, Venue
...
class MultiVenueOrderExecutor:
    ...
    async def execute(
        ...
        if notional_quote <= 0:
            return {"ok": False, "venue": venue.value, "message": "Notional must be positive"}

        cfg = DEFAULT_VENUES.get(venue)
        if cfg and notional_quote > cfg.max_notional_per_order:
            msg = (
                f"Requested notional {notional_quote:.2f} exceeds per-order cap "
                f"{cfg.max_notional_per_order:.2f} for {venue.value}"
            )
            logger.warning("Guardrail: %s", msg)
            return {"ok": False, "venue": venue.value, "message": msg}
